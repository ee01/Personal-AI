...
import {
  DesktopAppClient,
  type DesktopAppHealth,
  type DesktopAppStatus,
} from '../services/DesktopAppClient';
...
const APP_HINT =
  '在 Applications 中打开 “Personal AI.app” 完成登录、线程绑定和 Memory Service 配置。后台同步默认开启，关闭窗口即可继续运行。';
...
export const DesktopAppPanel: React.FC = () => {
  const [bridgeClient] = useState(() => new DesktopAppClient());
  ...
  const [message, setMessage] = useState('正在检查本机 Desktop App...');
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setMode('success');
      setMessage(
        statusResult.browserRunning
          ? 'Desktop App 正在运行'
          : 'Desktop App 已连接，但后台未运行',
      );
    } catch (error) {
      setHealth(null);
      setStatus(null);
      setMode('error');
      setMessage(
        error instanceof Error ? error.message : '无法连接 Desktop App',
      );
    } finally {
      setBusy(false);
    }
  ...
  const summaryItems = [
    ['App 版本', status?.appVersion || health?.version || '-'],
      ...
      <div className="desktop-app-shell">
        <header className="hero">
          <div>
            ...
            <p>
              Chrome extension
              中的这个页面现在只负责检测本机服务、提示缺失步骤，并引导你去
              `Personal AI.app` 中完成真实配置。
            </p>
          </div>
        ...
        <section className="panel grid-2">
          <div className="card">
            ...
            <ol className="guide-list">
              <li>
                {
                  '下载并安装最新的 `Personal-AI-Desktop-<version>-Installer.pkg`。'
                }
              </li>
              <li>在 Applications 中打开 `Personal AI.app`。</li>
        ...
        <section className="panel grid-2">
          ...
          <div className="card">
            ...
            {status?.blockingReasons?.length ? (
              ...
              <div className="reason-pill success-pill">
                当前没有检测到阻塞项，自动同步应该已经可用。
              </div>
            )}
          </div>
        </section>
        ...
        <section className="card">
          ...
          <ul className="guide-list">
            <li>
              这个 extension 页面不再配置本机服务地址、token 或自动同步开关。
            </li>
            <li>
              Desktop App 本机服务默认固定运行在 `http://127.0.0.1:46321`，由
              app 自动管理。
            </li>
            <li>
              真正的登录、线程绑定和同步节奏调整，都应该在 `Personal AI.app`
              中完成；后台同步默认开启。
            </li>
          </ul>
        </section>
...
export { DesktopAppPanel as DoubaoBridgePanel };
