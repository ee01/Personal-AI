...
| 文件 | 用途 | 调用方 | Env / Key |
|---|---|---|---|
| `agent-task-jumpboard.yml` | Jira → Dify → `memory-service /api/v1/agent-tasks/execute` | Executor Rule `pushMethod=AgentTask` | `AGENT_TASK_DIFY_API_*` |
...
- secret 环境变量导入 YAML 时通常为空，Botman / RingCentral 相关 token 要在 Dify 环境变量里手工补齐后再 Publish。
