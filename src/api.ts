...
export function sendDataToOllama (data: any[], config: IConfig) {
  ...
  const concerned_parts = localStorage.getItem('concernedItems') || [
    ...
    '任何明确 @ 我的账号名字的消息，或者提到账号名字的消息',
  ];
  console.log(data);
  const messages = data.reduce((acc, item) => `${acc}\n
    <team>
      <team_name>${item.groupName}</team_name>
      <team_id>${item.groupId}</team_id>
      <posts>${item.posts.map((post:any) => `
        <post>
          <sender>${post.creator}</sender>
          <post_content>${post.text}</post_content>
    ...
    // 增加调试数据👇
    + `\n\n    <team>
        <team_name>Recording Test</team_name>
        <team_id>123</team_id>
        <posts>
          <post>
            <sender>Sophia (Jinmei) Lin</sender>
            <post_content>Recording project BE dependencies completed</post_content>
            <datetime>2025-02-13 00:00:00</datetime>
          </post>
        </posts>
      </team>`
    + `\n\n    <team>
        <team_name>大群</team_name>
        <team_id>321</team_id>
        <posts>
          <post>
            <sender>Colin Liu</sender>
            <post_content>@Team 应要求，大家注意一下到公司时候的上下班时间，至少保持8个小时在公司的时间，无特殊情况不要中场离开，谢谢各位 。</post_content>
            <datetime>2025-02-14 00:00:00</datetime>
          </post>
        </posts>
      </team>`
    + '\n    </messages>';
  ...
  const prompt = `
    我的账号名字是：${username} （提供判断消息是否有@我，即便是不带姓氏@名字部分 也视为提及）
    这是我预先定义的关注项 concerned_part（必须严格匹配以下几项）：
    - ${concerned_parts.join('\n- ')}
    ...
    1. 认真分析以上消息，每条 team 都是同一个群组的消息集合，其中可能包含了多条 post，请逐个检查每一条 post，看看是否有可能是和我的 concerned_parts 相关的信息。
    如果有，第一步请将这个 <post> 所在 <team> 的数据原封不动的提取出来，你只帮忙过滤提取出符合条件的 <team> 数据，不要有其他额外的文字。
    以同样的XML格式返回`
  ...
  //     "concerned_part": "{concerned_part}", // 与先定义关注项 concerned_parts 中的某一项或多项目(分号隔开)
  ...
  //     - 并且这条返回记录的 concerned_part 字段设为你认为关联到的 concerned_parts 中的那一条的内容
  ...
  //   <新增验证层>
  //     <检查步骤>
  //       <步骤1>确认 concerned_part 是否是预先定义的关注项 concerned_parts 中的其中一项</步骤1>
  //     </检查步骤>
  //     <通过条件>同时满足步骤1</通过条件>
  //     <不通过处理>
  //       <检查步骤>
  //         <步骤2>如果不满足步骤1，检查 concerned_part 是否与 concerned_parts 信息关联</步骤2>
  //       </检查步骤>
  //       <通过条件及处理>同时满足步骤2，则将 concerned_part 改为关联的 concerned_parts 信息</通过条件及处理>
  ...
  //   - 返回的数组中每条记录不应该超出我的关注范围，其中 {concerned_part} 的值只能是是预先定义的关注项 concerned_parts 中的其中一项，不要创造新的关注项
