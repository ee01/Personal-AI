...
${process.env.LLM_GROUP_ANALYSIS === 'true' ? '' : '每条 <message_group> 都是同一个群组的消息集合，其中可能包含了多条不同人发的 <message_content>，不同的 <message_group> 不相关联。'}	

---- 以下是我的需求和你需要返回的内容定义 ----
${process.env.LLM_GROUP_ANALYSIS === 'true' ? '针对消息内容' : '让我们来一个一个查看 <message_group>，并且针对每个 <message_group> 都' }执行以下三步的任务：
1. 请仔细阅读 <message_group> 里的每条聊天消息，判断里面的 <message_content> 是否有符合以下规则其中一条${process.env.LLM_GROUP_ANALYSIS === 'true' ? '' : '。如果没有则跳过并查看下一个 message_group'}：
...
${process.env.LLM_GROUP_ANALYSIS === 'true' ? '' : '结束当前 <message_group> 的三步任务后，开始遍历下一个 <message_group>，直到所有 <message_group> 都遍历完成。'}
...
`
	...
	if (process.env.LLM_GROUP_ANALYSIS === 'true') {
		// 拆分单条发送 LLM
		let countAnalyzed = 0;
