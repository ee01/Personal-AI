...
const sendMessageToLLM = async (prompt: string, messageData?: any) => {
    ...
    try {
        ...
        if (isBackground) {
            ...
            const response = await reviewMessageByLLMAndSendToBot({ prompt, messageData });
            return response;
        } else {
        ...
        if (jsonArray && jsonArray.length > 0) {
            jsonArray.forEach(async json => {
                ...
                if (process.env.LLM_REVIEW_BEFORE_SEND === 'true') {
                  ...
                  const [reviewResponseRaw] = await handleLLMRequest({ prompt: reviewPrompt, model: process.env.OLLAMA_REVIEW_MODEL });
