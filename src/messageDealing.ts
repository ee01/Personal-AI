...
const sendMessageToLLM = async (prompt: string) => {
    ...
    try {
        ...
        if (isBackground) {
            ...
            const response = await reviewMessageByLLMAndSendToBot({ prompt });
            return response;
        } else {
        ...
        if (jsonArray && jsonArray.length > 0) {
            jsonArray.forEach(async json => {
                ...
                if (process.env.LLM_REVIEW_BEFORE_SEND === 'true') {
                  ...
                  const [reviewResponseRaw] = await handleLLMRequest({ prompt: reviewPrompt });
