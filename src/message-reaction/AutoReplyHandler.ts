...
function findAllMatchedConcernedItems<T extends { text: string; id?: string }>(
    ...
    const fallbackMatch = findMatchedConcernedItemByText(matchedRule, concernedItems);
    if (fallbackMatch) {
 ...
 * 通过文本匹配查找关注项
 */
function findMatchedConcernedItemByText<T extends { text: string }>(
    matchedRule: string | undefined,
    concernedItems: T[]
): T | undefined {
...
): Promise<{ handled: boolean; replyInfo?: { content: string; scheduleTime: Date; status: string; messageId?: string } }> {
    try {
        // 0. 前置检查：定时消息是否已初始化
