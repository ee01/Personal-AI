 ...
 * 保存消息交互功能配置
 */
async function saveReactionConfig(config: MessageReactionConfig): Promise<void> {
  try {
    const result = await chrome.storage.local.get(['envConfig']);
...
function hideToolbar() {
  if (currentMessageElement) {
    const toolbar = currentMessageElement.querySelector('.message-reaction-toolbar');
    if (toolbar) {
...
async function showSettingsPopup(anchorElement: HTMLElement) {
  ...
  // 绑定保存事件
  popup.querySelector('.snooze-btn-confirm')!.addEventListener('click', async () => {
    const snoozeCheckbox = popup.querySelector('[data-feature="snooze"]') as HTMLInputElement;
    const autoReplyCheckbox = popup.querySelector('[data-feature="autoReply"]') as HTMLInputElement;
    ...
    // 如果两个都关闭，显示提示
    if (!newConfig.enableSnooze && !newConfig.enableAutoReply) {
  ...
  const closeOnOutsideClick = (e: MouseEvent) => {
    ...
    if (!target.closest('.reaction-settings-popup') && !target.closest('.reaction-settings-btn')) {
      hideSettingsPopup();
...
function processMessageElement(messageElement: HTMLElement) {
  ...
  conversationCard.addEventListener('mouseenter', async () => {
    ...
    showTriggerTimeout = setTimeout(async () => {
      ...
      bindToolbarEvents(toolbar, targetElement, () => messageInfo, (info) => { messageInfo = info; });
    }, 3000);
  });
  ...
  conversationCard.addEventListener('mouseleave', (e: MouseEvent) => {
    ...
    const isMovingToToolbar = relatedTarget?.closest('.message-reaction-toolbar');
  ...
  console.log('💬 MessageReaction: 已为消息添加工具栏', targetElement.getAttribute('data-id'));
}
...
function bindToolbarEvents(
  ...
  // 设置按钮（x 按钮）事件
  const settingsBtn = toolbar.querySelector('.reaction-settings-btn') as HTMLElement | null;
  if (settingsBtn) {
...
export function initMessageReaction(config?: MessageReactionConfig) {
  ...
  if (!globalConfig.enableSnooze && !globalConfig.enableAutoReply) {
    console.log('💬 MessageReaction: 稍后处理和自动答复功能都已禁用，跳过初始化');
    return;
  }
