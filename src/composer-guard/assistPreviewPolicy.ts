import type { ComposerAssistResponse } from './types';

export type ComposerGuardPrimaryAction = 'insert' | 'preview';
...
export function shouldPreviewComposerAssistBeforeInsert(
  assist?: Pick<ComposerAssistResponse, 'previewRequired' | 'riskLevel'> | null,
): boolean {
  return Boolean(assist?.previewRequired || assist?.riskLevel === 'high');
}

export function getComposerGuardPrimaryAction(
  assist?: Pick<ComposerAssistResponse, 'previewRequired' | 'riskLevel'> | null,
): ComposerGuardPrimaryAction {
  return shouldPreviewComposerAssistBeforeInsert(assist) ? 'preview' : 'insert';
}
