...
export async function runClaimedTask(
  ...
  try {
    ...
    const promptText = [
      ...
      '{"status":"success|capability_missing|auth_error|need_human_decision|error","summary":"...","outcome":{"mode":"read|write","verdict":"observed|empty|mutated|noop","sourceSystem":"...","method":"...","subject":"...","count":0},"artifacts":[{"kind":"note","title":"...","content":"...","metadata":{}}]}',
      '先自己判断读/写是否做成。success 必须带 outcome，或 query_result（query+matchCount）/ 对象收据（entityKey+verification；读 observedFields，写 operation+changedFields）。展示用分组 note 不是成功条件。',
      '',
      'Task:',
      task.request?.task || '',
      '',
