...
- 紧凑 surface（会议弹幕、live feed chip）额外返回 `oneLineSummary`：优先取记忆侧的规范一句话 gist（`messages_raw.summary` 的 ingestion LLM 摘要、source memory 的 `oneLineCue`，其次 `metadata.summary`），只在缺失时让前端回退到 `uiSummary` / extractive preview。存储时产出 gist、提取时只做场景装帧，保证同一记忆能被不同口径复用，同时不给被动召回路径加 LLM 调用。message 和 chunk 都覆盖：chunk 通过 `chunks.file_path` / `related_entity_id` 回链到源 message 的摘要。
