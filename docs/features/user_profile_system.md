...
- 待确认推断队列: 页面把未确认条目按影响排序完整展示，并支持按待确认、证据覆盖筛选后就地确认或排除。
- 个性化边界提示: 条目列表会标出“可用于个性化”或“确认前不使用”，避免用户误以为未确认推断已经进入上下文。
- 可解释检查: 条目列表展示来源、证据数量和更新时间，推断内容带有类别和置信度。
- 数据导出: 展示页会限制拉取量以保持响应速度，导出路径会继续分页直到后端返回的画像条目全部写入 JSON，并在 `exportInfo.pagination` 中记录导出条目数、总条目数和是否截断，便于迁移、备份和审计。
...
- ChatGPT 的记忆管理正在强化搜索、排序、优先/降权和历史恢复；Personal AI 应继续把校准队列作为首屏任务，而不是只展示少量高分推断。
- Claude 已支持记忆导入/导出和项目级记忆边界；Personal AI 的导出必须避免被后端单页上限截断，后续可补充导入和按项目/场景分区。
- Claude Managed Agents 的文件式记忆和审计日志说明，生产级记忆系统需要可导出、可回滚、可追溯；Personal AI 的画像导出因此应包含分页完整性元数据，而不是只生成当前页面看到的列表。
