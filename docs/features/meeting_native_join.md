...
打开 native client 时，页面会显示一个 `Opening RingCentral app...` 兜底浮层。5 秒后如果原网页已经隐藏或失焦，系统认为用户已交给本机 app 加会并自动收起浮层；如果页面仍可见且聚焦，浮层会保留并提示 `Still on this page?`，继续提供 `Join in browser` 和复制链接。用户也可以点右上角 `x` 手动关闭。一旦用户选择浏览器加入、复制浏览器会议链接，或切换默认加会路径，浮层会进入手动恢复状态并取消自动消失，避免恢复操作进行中被收走。`Join in browser` 会先打开新的浏览器窗口、断开 opener，再跳到 Web 会场；如果浏览器拦截了新窗口，会自动改为在当前 tab 打开。浏览器兜底会直接打开 `https://v.ringcentral.com/conf/on/:meetingId`，避开 RingCentral `/launcher/:meetingId` 中间页。
...
- RingCentral、Teams 和 Zoom 都把浏览器加入作为无下载或失败恢复路径呈现；本功能保持 native 优先，同时把 Web fallback 做成短暂可见但可恢复的入口：已离开浏览器时自动收起，仍停在原页面时保留恢复按钮，避免 external-protocol 被取消或没有安装 app 时用户被卡住。
