...
2. 在命中规则后继续触发通知、关注后续、自动回复、摘要或自动化动作。
...
- `followThread`: 关注后续
- `automationPrompt`: 自动化动作描述
- `automationRequiresApproval`: 自动化动作是否需要批准
...
默认情况下，“操作无需批准”是勾选状态，意味着命中的自动化动作会尽量直接进入自动执行链路；如果用户取消勾选，则对应动作会以待批准方式入队。
...
- “记忆入口规则”
- “命中后是否写入记忆”
- “命中后是否附加通知 / follow-thread / digest / 自动回复 / 自动化动作”
