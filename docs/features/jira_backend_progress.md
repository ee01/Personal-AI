...
1. **同项目不展示**：依赖票与当前 issue 同项目时不展示（例如在 `NOVA-xxx` 上不会再把其他 `NOVA-*` 当成 BE 依赖）。
...
4. 按层 → 项目前缀映射匹配 Epic（例：Platform→`PLA`、Telco→`CNV`、Nova→`NOVA`、UX→`UX`/`UXAI`/`UXPHONE`）
...
验证示例：`NOVA-15209` Parent=`INIT-30072` → Required layers Platform/Telco/Nova/UX → 跨项目 Epic 如 `PLA-97920`、`CNV-88546`（同项目 Nova 子票会被同项目过滤掉）。
