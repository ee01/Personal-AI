...
- 用户可从卡片内将当前站点暂停提示 24 小时，或永久关闭该站点的网页记忆提示；状态存入扩展本地 storage。内容脚本会先读取站点控制状态，再发起被动召回。设置页的 Memory Service 区域也能查看、恢复单个站点或清空全部临时静默/永久屏蔽站点。
...
- 卡片提供“此网站今天不提示”和“永久不提示此站点”入口；点击后显示短暂确认，而不是只让提示突然消失。
- Options 页提供“网页记忆提示静默站点”管理入口，用户可以看到剩余静默时间、永久屏蔽站点，并恢复单个站点或全部站点。
...
- 目前有 24 小时站点暂停、永久站点屏蔽和设置页恢复入口，但还没有站点 allowlist 或按路径规则控制。
- 内容脚本已有 helper 级验证和浏览器脚本验证，仍缺常驻的完整站点适配器测试矩阵。

## 后续方向

1. 增加站点 allowlist 和按路径规则控制，尤其是隐私敏感页面。
2. 在 context recall 结果里展示更明确的来源类型和时间。
3. 为内容脚本增加更完整的站点适配器测试矩阵，覆盖 SPA 切换、关闭/不相关反馈和跳转链接。
