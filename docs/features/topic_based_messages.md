...
- 主题详情支持 `?readFilter=unread|read|all`，用于从主题列表直达未读阅读流；`?messageId=` 深链会自动切到聊天记录、解除搜索/状态筛选、展开对应讨论、短暂高亮并把该讨论标记为已读；如果命中的是上下文消息，会同时标出父讨论里的具体上下文行；深链和展开状态兼容 `id`、`messageId`、`conversationId`、`sourceMessageId` 以及常见导入数据里的 `message_id`、`conversation_id`、`source_message_id`；如果父讨论没有稳定 ID 但上下文消息有 ID，仍会用当前渲染位置定位；如果当前详情数据没有这条消息，会显示提示并回到全部聊天记录。
