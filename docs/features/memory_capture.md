...
- 普通整页候选先经过确定性 `source-memory/candidates/score`；只有 eligible 的新语义快照才调用一次聚焦 LLM，输出 `skip / remember / update_existing`、最多 4 条带正文证据的事实和最多 3 条行动项。被动路径不再进入 Agent Thinking 循环，不调用 Jira / 历史记忆工具，也不直接写入或通知。运行时会删除无法在当前正文核验的事实和实体；`skip` 强制清空事实、实体、补证提示和通知字段。完整正文语义 hash、content-script 去重、background single-flight 与 `chrome.storage.session` 缓存共同覆盖 DOM 抖动、刷新和同快照多标签页；focus 只刷新 Memory Lens，不触发 LLM。模型不可用时回退到原有确定性候选评分，保存仍只走下述复核 / 自动入库契约。
