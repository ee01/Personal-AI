...
- 普通整页候选先经过确定性 `source-memory/candidates/score`；只有 eligible 的新语义快照才调用一次 `POST /source-memory/webpage-analysis`，由 memory-service 当前已配置的后端模型执行固定、无工具的聚焦提示词，输出 `skip / remember / update_existing`、最多 4 条带正文证据的事实和最多 3 条行动项。浏览器不再直接调用 Dify/OpenAI，也不再进入 Agent Thinking 循环；接口不接受任意 prompt，不调用 Jira / 历史记忆工具，也不直接写入或通知。浏览器会再次删除无法在当前正文核验的事实和实体；`skip` 强制清空事实、实体、补证提示和通知字段。
