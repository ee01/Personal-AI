...
- 普通整页候选先经过确定性 `source-memory/candidates/score`；只有 eligible 的新语义快照才跑一次 LLM 聚焦分析——**在浏览器本地执行**，用用户在 Options「LLM」区块配置的模型（与消息分析共用同一份配置，见下方「网页分析的 LLM 路径」），提示词固定、无工具，输出 `skip / remember / update_existing`、最多 4 条带正文证据的事实和最多 3 条行动项。不进入 Agent Thinking 循环，不接受任意 prompt，不调用 Jira / 历史记忆工具，也不直接写入或通知。浏览器会再次删除无法在当前正文核验的事实和实体；`skip` 强制清空事实、实体、补证提示和通知字段。
- 完整正文语义 hash、content-script 阈值去重、background single-flight 与 `chrome.storage.session` 成功缓存共同覆盖 DOM 抖动、刷新和同快照多标签页。确定性候选只在会改变评分的信号变化时重算：首次有意图、复制状态变化、滚动跨过 60%、停留跨过 90 秒或正文语义实质变化；240/480 秒自动保存门槛复用已有评分，不再每 30 秒 POST。模型失败按同一快照持久化 5/10/15 分钟退避，冷却期沿用确定性候选；用户手动分析可绕过退避，但仍加入相同的 in-flight 请求。成功的非 skip 结果缓存 45 分钟、skip 缓存 20 分钟。
...
被动网页聚焦分析（判定 skip/remember/update_existing + 抽取带证据的事实）**只在浏览器本地执行，用用户在 Options「LLM」区块配置的模型**，和消息分析共用同一份配置（`src/llm.ts` 的 `handleLLMRequest`）。这是 2026-08-25 的最终决策，没有任何 memory-service 服务 key 兜底：
...
- `npm run eval:passive-webpage-analysis`（合成 blocker / decision chat / static shell / strict skip；不调用真实模型）
