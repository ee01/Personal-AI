...
- `/context-recall` 支持 `sourceTypes: ['source_memory']`，返回专门的 `source_memory` 资料记忆卡，并可跳到 Memory Exploring 的 capsule 详情页查看保存原因、证据锚点、草稿要点、未来触发线索和关联 web 记忆信号。
...
- 2026-05-30 检查结果：选中文本入库不再调用浏览器原生 prompt，而是在页面内打开轻量复核面板；取消只关闭面板并保留右侧 `+ 入库`，保存失败会停留在面板内显示错误。
