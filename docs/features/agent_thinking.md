...
- Options 里的流程图节点、运行检查涉及步骤、`Trace span 构成` 的问题步骤、待确认队列对应步骤、结果整理终止步骤和结果区审批定位按钮都同步了 hover / 读屏边界。
- 这些定位控件会说明复核理由，并统一标明点击只展开并聚焦当前页面时间线，不会批准、复制诊断包或审批文本、恢复 run、重跑、发送通知、写入、删除或执行外部动作。
