...
实现状态：Compose Assist 身份投影 P0 已于 2026-07-15 交付到 RingCentral、Jira 和 Web AI。当前行为、维护入口和验证命令以本文为准；后续跨模块 projection 仍按本文“身份投影合约”的边界独立评审。
...
1. 用户 focus 输入框时只建立会话；继续输入只让旧建议失效。只有一次真实 blur 才冻结当前草稿并请求，发送按钮、格式工具栏和 Personal AI 自己的控件不会误触发请求。
2. 后端先识别当前 scene。RingCentral/Jira 优先看当前会话、issue、可见字段和对象；Web AI 以冻结的 draft 作为 Prompt Compiler 主输入，再把直接相关记忆作为可选补充。
3. Context Recall 的最终 matches 先经过共享 [Evidence Cohesion Gate（证据对齐）](./evidence_cohesion_gate.md)；Compose 在真正消费前再执行一次，覆盖 change projection 和 locked-context fallback。RingCentral/Jira 生成可发送正文或受控 `draft_hint`；Web AI 先匹配高频确定性 patch，再由编译器在完整重写、局部补丁、上下文追加和静默之间选择。完整重写不要求先命中记忆，context pack 则必须有直接相关且已对齐的证据。
4. 每条候选都经过短生命周期 `PersonaProjection`。已确认人物关系优先于页面 hint；结构化画像只按当前场景裁剪，原始 `USER_CORE` 不进入 Compose。未确认、敏感、过期、scope 不明或无关的条目不能进入正文。
5. 候选还必须通过可写入文本、语言/目标保真、风险、置信度和当前草稿版本检查。没有正文、projection blocked、低置信或旧版本响应都不显示 Compose icon；仅有只读关联记忆时交给 Memory Lens。
6. hover 只给用户看最终待写入正文。`rewrite_prompt` 替换完整草稿，`prompt_patch/context_pack` 按选区追加；高责任场景先锁定预览。任何模式都只改草稿、不发送，并保留精确撤销和脱敏校准边界。
...
- `focusin` 只建立输入框会话，不发请求。
- `input` 只刷新 `draftRevision`、撤销旧预览、使在途响应失效，不发请求。
- capture-phase `focusout` 才冻结草稿快照并请求；同一个 `contextKey + draftRevision` 最多请求一次。相同草稿反复 focus/blur 不重复请求，修改后必须再次真正 blur。
- Web AI 草稿少于 8 个非空白字符时保持安静；RingCentral/Jira 可继续依靠 thread、conversation 或 issue 上下文处理空草稿。
...
- RingCentral/Jira 不把 draft 当作主召回信号；Web AI `compose_to_ai` 会把 blur 时冻结的 draft 作为 Prompt Compiler 输入和 enrichment signal。
...
- `rewrite_prompt` 必须使用 `replace_draft`：预览标题为 `优化后的完整提问`，确认按钮为 `替换原 prompt`，确认后替换 textarea、input、contenteditable 或 rich iframe 的完整内容，并派发 `inputType='insertReplacementText'` 和 `change`，光标落在末尾。缺失或错误的 replace 模式直接隐藏，绝不把完整重写追加到原文后面。
...
- 锁定复核态同样以待写入正文为主体；完整重写显示 `取消 / 替换原 prompt`，补丁或上下文显示 `取消 / 追加到 prompt`，普通回复显示 `取消 / 插入草稿`。不展示 `来源路由`、`草稿回执` 或 `建议依据`。如果是身份投影要求复核，正文下方只增加一行边界说明：确实排除了条目时显示“已按当前场景省略未确认或敏感身份信息；仅插入草稿，不会发送。”；只是对象/场景责任较高时显示“当前对象或场景要求先预览；仅插入草稿，不会发送。”。普通 hover 不显示这行。
...
- 用户在建议生成中或建议出现后继续编辑草稿时，前端立刻收起旧建议、推进 revision 并使旧响应失效；输入过程不重新请求，直到下一次真正 blur。
- 如果宿主编辑器直接替换 contenteditable DOM、没有正常派发 `input`，下一次重新聚焦或插入前校验仍会发现静默变化、清掉旧建议；只有后续真正 blur 才请求新版本。
...
- `compose_to_ai`：ChatGPT、Gemini、Claude、豆包等 Web AI 输入框。用户真正 blur 后，Personal AI 在输入框旁提供完整 prompt rewrite、局部 patch 或直接相关 context pack。
- `agent_compose`：预留给后续 Codex、Claude Code、Cursor 等 agent 入口。v1 先把这些 CLI agent 当作上下文来源，不在终端或 IDE 输入框里做浮层。
- Web AI 不再要求“必须命中记忆”。只要草稿足以表达完整任务，零记忆也可进入通用 Prompt Compiler，并返回 `rewrite_prompt + replace_draft`。
- 三种输出由后端强制映射：`rewrite_prompt -> replace_draft`；`prompt_patch -> append_patch`；`context_pack -> append_patch`。现有三个确定性高频 patch 先于通用编译器执行并继续 append，其余任务由一次结构化编译调用判断 rewrite、patch、context pack 或 none。
...
- 编译调用上限 1600 tokens、5.5 秒，`rewrite_prompt` 硬上限 6000 字符。当前 system prompt 进一步要求在 520 个 Unicode 字符内给出紧凑但有明确分段的结果；GPT-5 兼容模型使用 `reasoning_effort=none`，把交互延迟留给结构化生成而不是长推理。独立 kill switch `COMPOSER_PROMPT_COMPILER_ENABLED` 默认启用；关闭后仍保留确定性 prompt patch，且只有经过过滤的相关记忆才能降级成 context pack。
...
1. 召回并过滤 evidence。
2. 确定 reply、cue、`rewrite_prompt`、`prompt_patch` 或 `context_pack` 分支。
...
- Web AI rewrite/context pack/prompt patch 为 `context_pack_copyable`，始终预览。
...
- `title`, `url`
- `draftText`: blur 时冻结的用户草稿。RingCentral/Jira 不是主召回 query；Web AI `compose_to_ai` 同时把它用于任务编译和召回 enrichment。
...
- `available`: 是否应该展示建议。
- `suggestionType`: `none | context_pack | prompt_patch | rewrite_prompt | reply_context | issue_context`
- `insertMode`: `append_patch | replace_draft`。`available=true` 时必填；服务端按 suggestionType 推导，不接受模型自行决定。旧响应缺失该字段时，非 rewrite 按 append；rewrite 缺失或错误时前端隐藏。
- `insertText`: 可插入文本。
...
- `queryTimeMs`
- `debug`: 调试信息。Web AI / agent compose 重点看 `compiler.mode/gaps/usedEvidenceIds/outputLanguage`、过滤前后 evidence 数、`taskFrame`、`egressRisk` 与 `recall.contextExpansion`；这些诊断不进入用户可见正文。
...
2. 构造 `ContextRecallRequest`。RingCentral/Jira 的主 query 来自当前场景和 audience；Web AI 把冻结的 draft 放在召回 query 前部，但召回为空也不阻止完整 prompt rewrite。
3. `ContextRecallService` 走 fast path：`vector + fts`，limit 默认 3，并在最终展示前执行第一道 Cohesion Gate；随后执行 Web AI 低信息过滤、相关性准入、来源/id/内容去重和最多 3 条限制。如果显示预算把 matches 静音，但 context match 已锁定并携带 evidence ids，会先解析最多 6 条原始候选，再复用同一过滤链，最终仍最多 3 条。
4. Compose 对最终 evidence 执行第二道 Cohesion Gate，覆盖 change projection 和 locked-context fallback；后续分支只能读取 gated evidence。
5. RingCentral/Jira 继续执行严格场景相关性过滤，并生成可发送正文；LLM 超时或不可用时返回 `available=false`。
6. Web AI 先执行三个确定性 prompt patch。命中时直接返回 `prompt_patch + append_patch`，不调用通用编译器。
7. 其余 Web AI 任务在 kill switch 开启时进行一次结构化 Prompt Compiler 调用，只接收 `mode、insertText、usedEvidenceIds、gaps、confidence`。system prompt 明确要求只优化 prompt、保持草稿语言、保留目标/事实、不编造个人信息/引用，并把记忆视作未验证上下文；GPT-5 请求使用 `reasoning_effort=none`，结果保持紧凑以满足 5.5 秒交互预算。
8. 服务端校验 mode、文本长度、置信度、主语言、用户目标/事实保留和 evidence id；失败、非法 JSON、超时或语言不匹配一律 `available=false`。
9. 服务端把 mode 强制映射成 insertMode，只返回编译器实际使用的 evidence，并重新计算风险。未成年人、健康、家庭、发育等内容为 high risk；普通项目记忆仍是 medium。对于“任务已明确、只缺 evidence 上下文”的非研究请求，服务端还会把模型的完整 rewrite 归一化成 evidence-only context pack。
10. kill switch 关闭时不运行通用编译器；确定性 patch 仍可用，过滤后有高质量 evidence 才可生成简洁 context pack，不恢复旧任务判断/工具推荐模板。
11. 前端仍套用自适应置信度、预览和当前 draft revision 校验；写入后只改变草稿，不发送或提交。
...
- 用户在 Web AI 输入框里写了至少 8 个有效字符的 prompt，并在完成一轮输入后真正失焦。
...
- `draftText` 在 Web AI 场景既是 Prompt Compiler 的主输入，也是 recall enrichment signal；输入期间不请求，blur 时冻结。RingCentral/Jira 仍不让 draft 污染主召回。
...
Compose Assist 可把 [变化脉络](./change_memory_ledger.md) 投影转成既有 `source_memory` evidence。它只提供当前/历史边界和最近变化，不新增 UI 来源面板，也不改变 Compose 的 blur、risk、preview、insert、undo 和 send 边界。
...
- API 与服务编排：`memory-service/src/routes/composerAssist.ts`、`memory-service/src/core/ContextAssistService.ts`。
- 身份投影：`memory-service/src/core/ComposerAudienceResolver.ts`、`memory-service/src/core/PersonaProjectionService.ts`。
- 输入框探测、scene 和写入实现：`src/composer-guard/siteContextAdapters.ts`、`src/composer-guard/ComposerGuardController.ts`。
- 展示门、插入模式和回执文案：`src/composer-guard/assistPreviewPolicy.ts`、`src/composer-guard/types.ts`、`src/services/MemoryServiceClient.ts`。
- 质量回归：`memory-service/src/__tests__/api-composer-assist.test.ts`、`memory-service/src/__tests__/composer-assist-eval.test.ts`、`memory-service/src/__tests__/personaProjection.test.ts`、`tools/verify-compose-assist-*.mjs` 和 `evals/cases/compose-assist/`。
...
```bash
npm --prefix memory-service test -- --run src/__tests__/personaProjection.test.ts src/__tests__/api-composer-assist.test.ts src/__tests__/composer-assist-eval.test.ts
...
- Web AI 少于 8 个有效字符不请求；达到门槛后 focus/input 仍为零请求，真正 blur 才请求一次，相同 revision 重复 blur 不重发。
...
- ChatGPT/Gemini/Claude/豆包输入框 focus/input 期间不请求；blur 后有建议时只显示 Personal AI icon，不给输入框加红色 glow。
...
- 用户在旧建议请求未返回前继续输入时，不渲染也不能插入旧草稿版本的建议；输入停下仍不请求，下一次真正 blur 后才展示基于最新 revision 的建议。
...
- `previewRequired=true` 或 `riskLevel=high` 时，第一次点击 icon 只展开锁定正文预览；未点击对应的 `替换原 prompt / 追加到 prompt / 插入草稿` 前不能改写草稿，点击 `取消` 只关闭当前建议；锁定预览不展示来源名、标题、命中原因或“建议依据”列表。
