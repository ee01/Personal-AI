...
保存样例入口会显示“保存样例容量”回执：本地最多保留 12 条样例；保存同输入只更新这条样例，不会挤掉其他样例；保存新输入且已达上限时，会在点击前和保存后说明最旧样例被移出。本回执只影响本地回归样例集，被移出的样例不再参与批量回归、基线对比或导出报告，但不会删除 Memory Service 记忆、发送通知或执行规则自动化。

删除保存样例后会显示“保存样例删除回执”：它说明被移出的是本地保存样例及其本地基线、剩余样例数和下一条选中样例；删除只改写 `chrome.storage.local.agentWorkflowSavedScenarios`，不会删除 Memory Service 记忆、移除真实消息、发送通知、执行规则自动化、撤销已导出的报告或改写当前测试输入。

最近消息回放的来源回执会单独区分刷新中、空结果、读取失败和已选中样本：刷新 / 空结果 / 失败都只是 Memory Service time 通道的只读快照，不证明没有相关线上消息，也不代表当前聊天页、群组或时间窗口已覆盖；回放仍不会写入 Memory Service、发送通知、执行规则自动化、标记原消息已读或覆盖保存基线。
...
运行测试、填入/运行内置样例、最近消息填入/回放/刷新、保存样例、运行保存样例、批量回归、删除样例、单条/批量基线写回、导出报告和复制证据包这些控件本身也带 `title` / `aria-label` 边界。点击前就能知道它们只会触发本页本地测试、只读召回、`chrome.storage.local` 保存样例/基线、下载 JSON 或写本机剪贴板；不会写入 Memory Service、发送通知、执行规则自动化、标记原消息已读、接入外部 adapter、导出原始消息正文或把报告/基线动作混在一起执行。
...
- Options 的“运行就绪检查”只判断当前本地测试样例、trace、基线和 Agent 配置是否足以作为回归证据；它不会创建或更新 runtime contract。真实消息后续如果创建 `delegate_openclaw`，dispatch 时另由 [Action Readiness Contracts](./action_readiness_contracts.md) 检查 OpenClaw 鉴权、目标能力、必填输入和 proof，两个“就绪”不能互相代替。
