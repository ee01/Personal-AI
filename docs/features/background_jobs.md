...
`BackgroundJobs` 是扩展后台作业的统一调度入口，负责管理 Chrome alarm、作业开关、手动执行和状态持久化。它运行在 `background.ts` 中，面向 popup、记忆入口等 UI 暴露消息 API。产品名是「后台作业」，避免和任务中心混淆。

当前实现遵循 Manifest V3 的约束：

- alarm 监听器在 `background.ts` 顶层同步注册，避免 Service Worker 冷启动时漏事件
- 作业开关、上次执行时间和下次执行时间存储在 `chrome.storage.local.backgroundJobStates`；仍会读取旧键 `taskSchedulerStates`
- 新创建的 alarm 使用 `background_job_*` 前缀；启动、刷新和触发时仍识别旧的 `scheduled_task_*`，并在成功创建新 alarm 后清掉同作业的旧名
- 创建 alarm 时会优先显式设置跨会话持久化；如果当前 Chromium 尚不支持该字段，会自动降级到默认 alarm 行为。即便浏览器重启后 alarm 被清掉，Service Worker 启动和状态刷新也会按 Storage 状态重建
...
- 读取旧版/部分写入的 `backgroundJobStates` / `taskSchedulerStates` 时，如果某个作业缺少 `enabled` 字段，会回退到作业定义默认值，而不是把默认启用作业误判为关闭
...
- popup 点击重排后，在后台 `CONTROL_BACKGROUND_JOB` 和随后的状态刷新确认前，任务行保留原排程异常，并显示 `重排确认中` 回执；不会先把任务乐观改成已排程或伪造 `nextRun`
...
`background.ts` 仍有少量非 `BackgroundJobs` 的专用 alarm，例如 `cleanupFollowThreads` 和 `pollBackendNotifications`。这些不是后台作业 alarm，但创建时也会先检查现有 alarm，避免 Service Worker 每次唤醒都重置下一次触发时间。

## 大白话运行逻辑

Background Jobs 是扩展后台的“闹钟管家”。它不负责具体业务怎么做，只负责根据作业定义和用户开关创建 Chrome alarm、恢复丢失的 alarm、记录最近执行结果，并让 popup 能手动执行或修复。

结果主要受这些因素影响：

1. 任务定义默认值：每个任务的默认启用状态、间隔和执行函数是基础。
2. `chrome.storage.local.backgroundJobStates`：这是开关、上次执行、下次执行和历史结果的真源。旧键 `taskSchedulerStates` 只作读取兼容。
...
调用该 API 时，background 会先确保 `BackgroundJobs` 已启动，并实时核对 `background_job_*` / 兼容的 `scheduled_task_*` alarm。若启用作业的 alarm 丢失或间隔不一致，会按 Storage 状态自动补齐/修复，再返回最新 `nextRun`。
...
```ts
import { onTaskEnabledChanged } from './services/backgroundJobDefinitions';
...
监听器和 `getTaskEnabled()` 使用同一套默认值规则：如果 storage 里没有该作业、该作业缺少 `enabled`，或整组状态被清空，都会按作业定义默认值返回。依赖静默消息分析状态的 popup / 规则页应使用这个 helper，避免直接解析 `backgroundJobStates` 后在 reset 场景里显示旧状态。

## UI 入口

Popup 顶部保留静默消息分析的快捷开关。开关现在会等待 `CONTROL_BACKGROUND_JOB` 的真实返回结果，失败时回滚状态并显示错误。后台确认前会显示 `排程启用/停用提交中` 回执，说明当前仍是上次确认状态，尚未执行作业、确认 Chrome alarm、更新下一次执行时间或清空历史。
...
- 展开后台作业概览时会立即刷新状态；面板保持打开时会定时刷新，避免相对倒计时和排程异常判断变陈旧
...
- 单独启用/停用作业时，任务行先显示 `启用确认中` / `停用确认中`，并保留上次确认的排程状态、`nextRun` 和历史；只有后台 `CONTROL_BACKGROUND_JOB` 成功且刷新后的状态返回后，才显示已启用/已停用
...
- 新增后台业务任务时，优先加入 `src/services/backgroundJobDefinitions.ts` 的 `BACKGROUND_JOB_DEFINITIONS` 和 `src/services/BackgroundJobs.ts` 的 `executeTask()`
- popup、content script 或普通 UI 只需要读取任务开关时，导入 `src/services/backgroundJobDefinitions.ts`，不要直接拉入完整后台调度器
- `background_job_*` alarm 只能由 `BackgroundJobs` 创建和清理；旧的 `scheduled_task_*` 只作兼容识别
- 长耗时或可能产生副作用的作业需要保证可重复执行；调度器会跳过同一作业的并发重复触发
...
- 旧的 `CONTROL_SCHEDULED_CHECK` 已废弃。新代码使用 `CONTROL_BACKGROUND_JOB`；后台仍接受旧消息名 `CONTROL_TASK` 和 `GET_TASK_SCHEDULER_STATUS`
...
- [Temporal Web UI](https://docs.temporal.io/web-ui) 和 [Event History](https://docs.temporal.io/workflow-execution/event) 把当前状态、历史事件和调试入口放在一起；Background Jobs 对应保留最近运行历史和行内下一步，而不是只给一个开关。
- [GitHub Actions workflow runs API](https://docs.github.com/en/rest/actions/workflow-runs) 把 `queued` / `in_progress` / `completed` / `skipped` / `failure` 等状态和筛选口径拆开；Background Jobs 对应区分执行状态、排程健康、跳过、失败和刷新校准结果。
- [Apache Airflow UI](https://airflow.apache.org/docs/apache-airflow/stable/ui.html) 用 Grid View 展示最近运行和任务状态，并支持从失败/重试状态进入排障；Background Jobs 的 popup 也应优先展示需处理任务、失败原因和操作影响。
...
- [Datadog Monitor Status Page](https://docs.datadoghq.com/monitors/status/status_page/) 把告警原因、上下文和 quick actions 放在同一调查页面；Background Jobs 的 popup 面板也应把下一步和按钮副作用放在同一任务行。
...
2. 打开 popup，展开后台作业概览，确认能看到 8 个作业
...
7. 在 Service Worker 控制台检查 `chrome.alarms.getAll()`，确认 `background_job_*` 数量与启用作业一致（旧安装可能短暂仍有 `scheduled_task_*`，刷新后应迁走）
8. 对一个 `overdue` 任务点击重排，确认后台响应前任务行显示 `重排确认中` 且仍保留原异常快照；后台确认后 `nextRun` 回到未来时间，且不会更新 `lastRun`
9. 运行 `npm run verify:background-jobs-api`，覆盖首次启动不安排隐藏执行、启用不立即执行、storage 缺 row/清空时回退任务默认值、alarm 显式跨会话持久化、alarm 创建失败回滚、手动执行、失败记录、跳过记录、最近运行历史、重复执行跳过、停用任务手动执行、状态刷新补齐丢失 alarm、自动修复失败时仍返回任务列表、状态刷新回执、具体 alarm 校准任务明细、队列明细读取失败回执、重排失败时保留旧 alarm、识别并修复明显滞后的 alarm，以及清理旧版本遗留的未知 `scheduled_task_*` alarm
10. 运行 `npm run verify:background-job-status-filters`，覆盖后台作业状态计数、状态优先级和状态分类判定
11. 运行 `npm run verify:background-jobs-popup:e2e`，用 fresh Chromium 扩展实例验证 popup 折叠态需处理作业预览、展开后不显示筛选标签、需处理总览、作业列表优先级、上次确认时间、刷新中 `正在核对` 回执、刷新范围回执、具体 alarm 校准明细、刷新失败 `刷新未确认` 回执和旧快照边界、队列明细未确认回执、停用/立即执行/重排确认中都不提前改成成功状态、启停/重排失败操作回执、行内操作范围、最近一次运行解释、失败详情呈现、确认后的操作回执、英文静默分析文案和右上角 Help Center / Desktop App 图标顺序
