...
1. 手动规则匹配：用户配置的关注项是通知、摘要和自动化动作的主要触发来源。同一条消息可以同时命中多条即时通知规则；推送里的「关注项」会把这些规则拼在一起，开启了 @我 的规则会标成（@提醒），只要其中一条开启 @我 就会 mention。
...
即时通知分两个互相独立的推送场景，各自有独立的目标配置和 Bot 模板：

| 场景 | 触发 | 推送目标配置（Options） | Bot 模板 |
| ---- | ---- | ----------------------- | -------- |
| 消息分析推送 | 命中普通关注项 | 「消息分析推送」`MESSAGE_ANALYSIS_PUSH_TARGET` / `MESSAGE_ANALYSIS_PUSH_GROUP_ID` | `__关注项__` + “监测到您可能关注的消息” |
| 关注后续推送 | 命中带 `followThread` 的关注后续规则，或 LLM 返回 `follow_thread_info` | 「关注后续推送」`FOLLOW_UP_PUSH_TARGET` / `FOLLOW_UP_PUSH_GROUP_ID` | `📌 关注后续更新` + `__关注话题__` + “监测到的「关注后续」新动态” |

关注后续规则命中会统一路由到 `follow_up` 场景：无论 LLM 是通过 `follow_thread_info` 确认关联，还是只在 `matched_rule` 里命中该规则，都不会落到消息分析目标或消息分析模板上。每小时合并的关注后续汇总使用独立的 `buildFollowThreadDigestBotNotificationMessage` 模板。详见 [关注后续通知内容](./message_reaction.md#通知内容)。
...
同一条消息可以同时命中多条手动规则。摘要分发会按所有启用摘要的命中规则分别入队；即时通知会收集全部非摘要且配置了通知渠道的命中规则：渠道仍按第一条发送，但 Glip「关注项」会换行拼接这些规则，开启 `mentionMe` 的规则排在前面并带 `（@提醒）`，mention 只要其中一条开启就会触发。因此“摘要-only”规则不会吞掉同一条消息上的其他即时通知规则，用户也不会只看到一条 `mentionMe: false` 的规则却被另一条关联规则 @。发送前的 LLM 审核只能把规则文案合并进这份列表，不能覆盖掉已命中的其他规则。
...
- 自动执行中的 OpenClaw 动作如果本地 `agent.wait` 超时，会按 30s/60s/120s 最多确认 3 次远端 `remoteRunId`；仍在跑则保持 `running`，3 次都对不上才 `dead_letter`。没有 `remoteRunId` 的旧路径仍按 `openClawTimeoutMs + 60 秒` 回收。系统不会自动重试外部写操作。
...
本轮复查发现，规则页顶部的 `立即启用` 会直接把本地状态切成“后台记忆采集运行中”，但用户看不到 Background Jobs 是否真的确认了开启。如果后台控制失败，手动规则可能被误读为已经会自动观察后续消息。

当前规则页已补齐：

- 点击 `立即启用` 后先显示 `后台采集开启回执 · 提交中`，说明确认前这些手动规则仍不能当作自动运行。
- Background Jobs 返回成功并重新读取到 `message_analysis` 已开启后，回执切到 `已确认`，状态条再显示后台记忆采集运行中。
...
- 即时通知收集全部非摘要且配置了 `notifyMethod` 的命中规则：渠道仍按第一条发送，「关注项」拼接全部规则，mention 取任一 `mentionMe`。
...
## 2026-08-18 更新：即时通知拼接多条命中关注项

本轮复查聚焦 Glip 推送的「关注项」归因。旧运行时即时通知只取第一条命中规则的 `mentionMe`，发送前 LLM 审核还可能把 `matchedRule` 覆盖成另一条规则原文。结果是：消息同时命中「AI 相关讨论」和「Personal AI 讨论」时，推送只显示前者（未开 @我），用户却可能被后者 @。

当前实现已收敛为：

- 即时通知列出全部非摘要命中规则，换行拼进「关注项」。
- 开启 `mentionMe` 的规则排在前面，并带 `（@提醒）`。
- mention 由全部即时命中规则的 `mentionMe` 取或。
- LLM 审核通过后只把返回文案合并进现有关注项列表，不再整段替换。

产品依据仍然是多规则触发必须可解释：用户看到的关注项应能解释这次为什么被 @，而不是只留下审核模型挑中的单条规则。
...
- 规则页监听 `backgroundJobStates`（兼容旧键 `taskSchedulerStates`），外部作业开关变化后状态条能同步更新。
...
- Background Jobs 会用这个回执生成任务历史摘要；只要存在记忆写入、即时通知、关注后续或联动规划失败，就把这轮后台作业标为部分失败。
- Popup / 调度状态里的失败详情会包含本轮分析条数、写入接收数、重复数、即时通知、摘要、联动、范围拦截和下游失败拆分。
- 这不会重跑分析、重发通知、标记消息已读或执行外部动作；它只是把已经发生的分发结果带回调度状态，避免后台作业把局部失败包装成成功。

产品依据继续沿用触发器 + 条件 + 动作的显式心智：Slack keyword workflow 要先指定 channel 与 keyword conditions，Zapier Filters 只让满足条件的项目继续执行。Trigger-Action Programming 调试研究指出，用户需要能看懂规则为什么触发或失败；Attention-Sensitive Alerting 则强调通知路径要权衡信息价值和打断成本。因此后台作业状态也必须继承分发回执，而不是只告诉用户“分析跑完了”。
