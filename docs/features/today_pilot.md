...
- 建议带进会议的问题；按 Options 界面语言生成，Video Home 用列表展示。
- 相关风险或未关闭事项。
- 证据来源。
...
Video Home 会在会议详情里显示一条 `会前准备回执`，用芯片标出准备模式、可见高置信证据数和基础背景数。日历-only 或 fallback meeting prep 仍可带入 Meeting Pilot 作为会议背景，但页面必须明确说明“高置信记忆 0 条”或“规则 fallback”，不能把基础准备误写成完整记忆召回，也不能把有效的基础准备展示成纯空状态。回执不再重复解释“仅命中日历/基础信息”这类用户已经能从芯片读出的命中说明。

卡片不再重复展示日历里已有的会议标题、时间、组织者和参会人数，也不再展示只是复述日历标题/组织者/参会人/参考材料的 summary。建议带进会议的问题按用户 Options 语言生成，并在卡片里用列表展示；旧缓存如果把多问句写在同一段，页面仍会按问号拆成列表。
