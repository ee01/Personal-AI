...
| Ask 主动问答 | Memory Service | [ask.md](./ask.md) | 主动问答入口，覆盖话题锁定、召回优先级、Ask 本轮状态、证据来源回执、证据守望、证据缺口和活答案沉淀 |
...
| 证据守望契约 | Memory Service | [evidence_watch_contracts.md](./evidence_watch_contracts.md) | 可变化事实进入 contract；列表、详情和 run history 都显示只读快照边界，详情/run history 带复核时间基准，Ask/Reflection/Action Queue 复用同一守望对象并把重复查证标成未复核来源 |
...
| 智能资料录入 | Memory Coverage Map | [memory_coverage_map.md](./memory_coverage_map.md) | 粘贴/文档/普通 zip 先显示范围回执，主按钮 hover/读屏说明 dry-run/提交边界，提交中保留写入回执 |
...
| Source Memory 蒸馏器 | Memory Capture | [memory_capture.md](./memory_capture.md) | 保存/补备注后蒸馏 ready cue、compact memo、trigger matcher、低副作用 links；详情页可补备注刷新并显示提交中边界 |
...
| 画像快速增强/降低影响 | User Profile | [user_profile_system.md](./user_profile_system.md) | 显式 importance 调整，按钮 hover/读屏说明目标权重、确认边界、服务确认和撤销范围 |
...
| Rehearsal 管理页 | Memory Exploring | [rehearsal.md](./rehearsal.md) | `memory-exploring.html#/rehearsals`，用于审计和修正；筛选/搜索/加载/恢复控件、列表卡片与详情动作按钮点击前显示只读、选择、写入、反馈和不执行边界 |
...
| Notification Center feed | Notification Center | [notification_center.md](./notification_center.md) | `GET /notification-center/feed`；meta 显示 limit 应用、快照和空结果口径，Provider / Doubao 非空摘要显示快照回执 |
...
| 联动操作 / Openclaw | Message Reaction | [message_reaction.md](./message_reaction.md) | 预填记忆入口规则，按钮点击前说明只开配置草稿；保存前执行预览和后续动作 |
...
| 主题静音 | Topic Messages | [topic_based_messages.md](./topic_based_messages.md) | 本地 mute 状态、静音边界、查看静音路径与取消静音回执 |
...
| App Script 自动更新 | Scheduled Messages | [scheduled_messages_manager.md](./scheduled_messages_manager.md) | deployments.update、版本探测、项目归属预检；检查/升级/恢复按钮 hover 与读屏标明只读、写入和回退边界 |
...
| Agent Thinking 分析编排 | Agent Thinking | [agent_thinking.md](./agent_thinking.md) | 通用工具/思考循环；终止到结果卡片之间显示整理回执、未处理问题摘要和终止步骤定位 |
...
| Agent Workflow 运行诊断 | Agent Workflow | [agent_workflow.md](./agent_workflow.md) | trace / storageReview / readiness；下一步动作显示本地排障边界，工具错误显示 Agent / Tool，证据包复制中锁定当前测试输入；运行/回放/基线/导出/复制控件 hover 与读屏说明本地测试、只读召回、剪贴板、下载和本地基线写入边界 |
...
| 会前准备 | Today Pilot | [today_pilot.md](./today_pilot.md) | calendar events / meeting prep；回执区分高置信记忆、基础背景、本机 handoff 和刷新补课结果边界；刷新和证据来源控件点击前说明无入会/录音/外发/写回边界 |
...
| Popup Top 3 | Today Pilot | [today_pilot.md](./today_pilot.md) | Chrome popup 今日摘要；溢出时可查看全部 mission |
...
| 记忆提示右下角关联记忆 | Memory Lens | [memory_lens.md](./memory_lens.md) | 当前网页/消息/Jira/会议上下文被动召回；展开卡显示页面召回回执 |
...
| 人脉关系 Context Card | Relationship Radar | [relationship_radar.md](./relationship_radar.md) | `/relationships/context-card`；复制、敏感范围切换和证据打开按钮点击前显示剪贴板 / 隐私范围 / 只读证据边界 |
...
| 项目证据修复路径 | Project Dashboard | [brain_like_project_analysis_system.md](./brain_like_project_analysis_system.md) | ETA、Jira、平台状态缺口；修复按钮说明本地打开/聚焦边界 |
...
| NC 加会浏览器回退 | Native Join | [meeting_native_join.md](./meeting_native_join.md) | 默认浏览器、复制完整链接、Meeting ID/passcode 手动恢复边界、全链路显示/隐藏与恢复按钮 hover/读屏边界、默认路径回执、规范化 app 重试和 fallback UI |
...
| secret value 脱敏 | Jira Automation Import | [jira_automation_import.md](./jira_automation_import.md) | `secret=true`、signed URL、function/API gateway query 凭据不展示 raw value，并按重录队列提示启用前处理顺序 |
...
| Persona / 近期重点 / 提醒推送 | Doubao Bridge | [doubao_bridge.md](./doubao_bridge.md) | 随手记导向格式；手动推送即时显示 package、条目、来源、线程、验证与失败未确认回执 |
