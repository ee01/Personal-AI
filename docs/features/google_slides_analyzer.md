...
- 支持表格型、文本/列表型内容；同一 slide 上多个可信项目表格会合并分析；表格备注列可识别 Comments、Highlights、Actions、Next steps 等常见命名；低置信或复杂结构可使用 LLM fallback。
...
- 写回区会预览即将写回的项目、字段和变更摘要；筛选隐藏的已选字段也会计入预览和溢出提示。
...
- 写回前必须让用户看到具体字段级变更预览，而不只展示数量；写回进行中不能继续改勾选状态。
