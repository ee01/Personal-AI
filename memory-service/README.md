...
| `CONTEXT_RECALL_PASSIVE_SEARCH_ENABLED` | `true` (omit = on) | Enable passive Memory Lens / browser recall search. Unset/missing enables it; only explicit `false`/`0`/`off`/`no` disables. Does not gate Compose Assist. |
...
| `COMPOSER_ASSIST_MODE` | `full` (omit = full) | Single operational switch for Compose Assist: `off` (no recall, no generation), `context_only` (recall but no insertable text), `full` (sendable drafts + Web AI compiler). Retired `COMPOSER_SENDABLE_GENERATION_ENABLED=false` maps to `context_only` for one release. |
