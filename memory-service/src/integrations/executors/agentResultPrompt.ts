...
export function buildAgentResultSystemPrompt(
  ...
  return [
    ...
    '- 读单个对象：sourceSystem + entityKey + verification + observedFields。',
    '- 写单个对象：sourceSystem + entityKey + verification + operation + changedFields。',
    '- 产出文件：kind=file，metadata.path（相对路径，不要 ..）+ verification。',
    '',
    '不能回读确认就不要 success。缺工具用 capability_missing；缺权限用 auth_error；需人选择用 need_human_decision，并带 payload.question / payload.options。',
    '',
    '例子（读扫描）：outcome.mode=read, verdict=observed, sourceSystem=jira, method=jql_requery, subject="filter=153978 AND status not in (Closed)", count=75；artifacts 里可再放各 Team 的展示 note。',
    '例子（读 0 匹配）：outcome.verdict=empty, count=0，或一张 query_result 且 matchCount=0。',
    '例子（写）：outcome.mode=write, verdict=mutated, count=4，且每个更新的 issue 一张收据：entityKey=NOVA-17023, verification=rest_api_readback, operation=update, changedFields=["Committed"]。',
    '例子（写 noop）：outcome.verdict=noop, count=0，subject 写实际跑过的查询。',
    'Jira 对象收据：metadata.entityKey=NOVA-123；metadata.url=你实际请求的 Jira 实例上的 browse/self 链接（从 API 的 self / html 取，不要编造主机名）；title=summary；有经办人则 metadata.assignee=显示名。',
    '例子（浏览器读）：sourceSystem=chrome, entityKey=页面 URL, verification=page_url, observedFields=["url","title"]。',
    '例子（文件产物）：kind=file, path="research/sqlite-vec-vs-lancedb.md", verification=file_write。',
    '',
