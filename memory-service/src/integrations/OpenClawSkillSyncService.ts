  ...
  async exportSkillPackage(slug: string): Promise<OpenClawSkillPackage> {
    const parsed = await this.requestReadJson(
      [
        ...
        'Include every regular text file under the matching skill directory that belongs to the skill, including package.json, scripts, references, resources, examples, templates, schemas, and fixtures.',
        'Only omit .git, node_modules, caches, binary files, generated build outputs, secret-bearing files, or files outside the skill directory.',
        'Do not summarize or replace file contents with ellipsis; if a required skill-local text file cannot be included, return ok:false with an error instead of a partial package.',
