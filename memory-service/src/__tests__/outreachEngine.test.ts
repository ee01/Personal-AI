...
describe('OutreachEngine', () => {
  ...
  it('combines multiple new replies from the same sender into a single stored reply', async () => {
    const session = outreachRepo.createSession({
    ...
    mockRingCentralListPosts('chat-multi', [
      ...
      {
        id: 'reply-3',
        text: '你自己看，video相关应该都在下面',
        creator: { id: 'user-42', name: 'Sophia (Jinmei) Lin' },
        creationTime: '2026-04-01T02:42:05Z',
      },
      {
        id: 'reply-2',
        text: "Gary's calendar",
        creator: { id: 'user-42', name: 'Sophia (Jinmei) Lin' },
    ...
    const updatedSession = outreachRepo.getSessionById(session.id);
    expect(updatedSession?.status).toBe('deferred');
    ...
    expect(replyEvents[0].payload?.replyText).toBe(
      "Gary's calendar\n你自己看，video相关应该都在下面\n他下周在杭州",
    );
    ...
    expect(replyMessages[0].content).toBe(
      "Gary's calendar\n你自己看，video相关应该都在下面\n他下周在杭州",
    );
  });

  it('does not re-process an already recorded reply post on later polls', async () => {
    const session = outreachRepo.createSession({
    ...
    const updatedSession = outreachRepo.getSessionById(session.id);
    expect(updatedSession?.replyPostId).toBe('reply-1');
    expect(updatedSession?.replyRawText).toBe('好');
  });
