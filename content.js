  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = (event) => {
      ...
      dataRequest.onerror = (event) => {
        console.log(`Error fetching data from store '${storeName}':`, event.target.error);
        reject(event.target.error);
      };
    };

    request.onerror = (event) => {
      console.log(`Error opening IndexedDB '${databaseName}':`, event.target.error);
      reject(event.target.error);
    };
  });
}
...
function TransformMessagePosts(enableMessage, startTime, selectGroupNames, ignoreGroupNames, selectFolderGroupIds) {
  if (!enableMessage) {
...
const TransformCalendar = (startTime, enableCalendar, username) => {
    if (!enableCalendar) {
        return Promise.resolve([]);
    }
