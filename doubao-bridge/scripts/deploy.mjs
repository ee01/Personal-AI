...
async function apiRequest(repository, method, resourcePath, body, token) {
  const response = await fetch(`https://api.github.com/repos/${repository}${resourcePath}`, {
    method,
    headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(`GitHub API ${method} ${resourcePath} failed: ${response.status} ${response.statusText}${text ? ` - ${text}` : ''}`);
  }
...
async function uploadAsset(repository, releaseId, uploadUrl, assetPath, token) {
  ...
  await fetch(url, {
    ...
    const text = await response.text().catch(() => '');
    throw new Error(`Uploading ${assetName} failed: ${response.status} ${response.statusText}${text ? ` - ${text}` : ''}`);
  });
}
...
async function main() {
  ...
  if (!token && ghInstalled) {
    ...
    await publishWithGhCli(repository, tagName, releaseTitle, releaseNotes, pkgPath);
  ...
  let release;
  try {
    release = await apiRequest(repository, 'GET', `/releases/tags/${encodeURIComponent(tagName)}`, null, token);
  } catch (error) {
  ...
  if (!release) {
    release = await apiRequest(repository, 'POST', '/releases', {
      tag_name: tagName,
      name: releaseTitle,
      body: releaseNotes,
      draft: false,
