...
function isExpandedState() {
  return state.uiState === 'pending' || state.uiState === 'streaming' || state.uiState === 'enriched';
}
...
function focusComposer() {
  ...
  elements.composer.setSelectionRange(elements.composer.value.length, elements.composer.value.length);
}
...
function renderStructuredAnswer(structuredAnswer) {
  ...
  if (typeof structuredAnswer.confidence === 'number') {
    sections.push(`<div class="confidence-badge">${formatConfidence(structuredAnswer.confidence)}</div>`);
  }
      ...
      <div class="evidence-list">
        ${displayItems
          .map((item) => {
            ...
            const sender =
              typeof metadata?.sender === 'string' && metadata.sender.trim() ? metadata.sender.trim() : '';
            const groupName =
...
function renderMemoryBadge(memorySaveResult) {
  ...
  const label = memorySaveResult.duplicate ? '这条我已经记住了。' : '好，这条我记下了。';
...
function setRuntime(runtime) {
  ...
  if (!runtime?.topStatus || !Array.isArray(runtime.items) || runtime.items.length === 0) {
  ...
  elements.statusPill.textContent =
    extraCount > 0 ? `${runtime.topStatus.label} +${extraCount}` : runtime.topStatus.label;
}
...
class VoiceController {
  ...
  describeError(code, payload = null) {
    ...
    return payload?.message || (code ? `语音输入暂时不可用：${code}` : '语音输入暂时不可用。');
  }
  ...
  handleNativeEvent(payload) {
    ...
    if (payload.type === 'transcript') {
      this.recognizedTranscript = typeof payload.text === 'string' ? payload.text.trim() : '';
    ...
    if (payload.type === 'stopped') {
      this.listening = false;
      this.recognizedTranscript = typeof payload.text === 'string' ? payload.text.trim() : this.recognizedTranscript;
...
function renderVoiceSheet() {
  ...
  elements.voiceOrb.classList.toggle('listening', state.voicePhase === 'listening');
...
async function submitQuery(rawInput, options = {}) {
  ...
  const standaloneRemember = rememberRequested && isStandaloneRememberRequest(input);
  let memorySaveResult = null;

  try {
    if (rememberRequested) {
      memorySaveResult = await rememberAck(normalizeRememberText(input) || input);
    }
  } catch (error) {
    memorySaveResult = {
      duplicate: false,
  ...
  if (standaloneRemember) {
    ...
    pushMessage({
      ...
      role: 'assistant',
      text: memorySaveResult?.error ? `记忆保存失败：${memorySaveResult.error}` : '好，这条我记住了。',
      htmlReady: true,
    ...
    await quickAsk.askStream(
      ...
      async (event) => {
        ...
        if (event.type === 'result') {
          flushStreamBuffer();
          finalResult = event;
          await new Promise((resolve) => window.setTimeout(resolve, ENRICHMENT_DELAY_MS));
          updateMessage(assistantId, {
            text: event.answer || '',
            pending: false,
            htmlReady: true,
