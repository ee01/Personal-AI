...
import { installManifest } from '../dist/nativeMessaging/manifestInstaller.js';
...
const shortcutHelperSourcePath = path.join(
  __dirname,
  'native',
  'shortcut-helper.swift',
);
...
const speechHelperSourcePath = path.join(
  __dirname,
  'native',
  'speech-helper.swift',
);
...
const keyStateHelperSourcePath = path.join(
  __dirname,
  'native',
...
const bridgeHost =
  process.env.DESKTOP_APP_HOST || process.env.DOUBAO_BRIDGE_HOST || '127.0.0.1';
...
const extensionIdsEnv =
  process.env.PERSONAL_AI_EXTENSION_IDS ||
  process.env.PERSONAL_AI_EXTENSION_ID ||
  process.env.DESKTOP_APP_EXTENSION_IDS ||
  process.env.DESKTOP_APP_EXTENSION_ID ||
  '';
const disableLoginItem =
  (process.env.DESKTOP_APP_DISABLE_LOGIN_ITEM ||
    process.env.DOUBAO_BRIDGE_DISABLE_LOGIN_ITEM) === '1';
const packagedPlaywrightBrowsersDir = path.join(
...
const ASK_WINDOW_VERTICAL_MARGIN = 80;
...
async function appendLog(filePath, line) {
  await fs
    .appendFile(filePath, `${new Date().toISOString()} ${line}\n`, 'utf8')
    .catch(() => undefined);
}

async function cancelPendingUninstall() {
  const previousMarker = await fs
    .readFile(uninstallMarkerFile, 'utf8')
    .catch(() => '');
  if (previousMarker.trim()) {
    await appendLog(
      appLogFile,
      '[info] cancelling pending uninstall because app was reopened',
    );
  }
...
function getBridgeEnv() {
  return {
    ...process.env,
    DESKTOP_APP_DATA_DIR: path.join(appSupportDir, 'data'),
    DESKTOP_APP_PROFILE_DIR: path.join(appSupportDir, 'profile'),
    DOUBAO_BRIDGE_DATA_DIR:
      process.env.DOUBAO_BRIDGE_DATA_DIR || path.join(appSupportDir, 'data'),
    DOUBAO_BRIDGE_PROFILE_DIR:
      process.env.DOUBAO_BRIDGE_PROFILE_DIR ||
      path.join(appSupportDir, 'profile'),
    TMPDIR: tempDir,
    TMP: tempDir,
    TEMP: tempDir,
...
function isCompatibleBridge(payload) {
  return (
    ['desktop-app', 'doubao-bridge'].includes(payload?.service) &&
...
async function ensureBridgeProcess() {
  ...
  if (
    existingHealth?.service === 'desktop-app' ||
    existingHealth?.service === 'doubao-bridge'
  ) {
    ...
    throw new Error(
      `Port ${bridgePort} is already in use by another local service.`,
    );
  }
  ...
  if (bridgeProcess.pid) {
    await fs
      .writeFile(bridgePidFile, String(bridgeProcess.pid), 'utf8')
      .catch(() => undefined);
  }
  ...
  if (!ready) {
    await appendLog(
      bridgeLogFile,
      `[warn] bridge did not become healthy within timeout at ${bridgeBaseUrl}`,
    );
  }
}
...
async function stopBridgeProcess() {
  ...
  if (!targetPid) {
    ...
    targetPid =
      Number.isFinite(parsedPid) && parsedPid > 0 ? parsedPid : undefined;
  }
...
function getAskWindowBounds(height = ASK_WINDOW_COMPACT_HEIGHT) {
  ...
  const x = Math.max(
    workArea.x + 16,
    Math.min(desiredX, workArea.x + workArea.width - width - 16),
  );
  const compactTop = Math.max(
    workArea.y + 12,
    Math.min(
      desiredCompactTop,
      workArea.y + workArea.height - ASK_WINDOW_COMPACT_HEIGHT - 12,
    ),
  );
...
function resolvePendingShortcutGesture() {
  ...
  if (gesture.actionOnTap === 'hide') {
    void appendLog(
      appLogFile,
      '[info] shortcut gesture resolved as hide-on-tap',
    );
    hideAskWindow();
    return;
  }
...
async function triggerPendingShortcutHold() {
  ...
  if (keyState.aDown && keyState.optionDown) {
    await appendLog(
      appLogFile,
      '[info] shortcut promoted to voice mode after confirmed A+Option hold',
    );
    sendToWindow(askWindow, 'quick-ask:native-shortcut', {
      type: 'enter-voice',
    });
    return;
  }
...
function beginPendingShortcutGesture(
  actionOnTap = askWindow?.isVisible() ? 'hide' : 'show',
) {
...
function createAskWindow() {
  ...
  askWindow = new BrowserWindow({
    ...
    transparent: true,
    roundedCorners: true,
    // Native shadow draws a rectangular halo behind rounded transparent content;
    // rely on CSS shadows inside the window for a softer float.
    hasShadow: false,
    movable: true,
    resizable: false,
    maximizable: false,
...
async function registerFallbackShortcut(message) {
  ...
  if (!registered) {
    await appendLog(
      appLogFile,
      '[warn] failed to register fallback shortcut Alt+A',
    );
  ...
  await appendLog(
    appLogFile,
    '[info] registered Electron globalShortcut Alt+A',
  );
  updateShortcutStatus({
    usingNativeHelper: false,
    fallbackEnabled: false,
...
function sendSpeechHelperCommand(command) {
  if (
    !speechHelperProcess ||
    speechHelperProcess.killed ||
    !speechHelperProcess.stdin?.writable
  ) {
...
async function scheduleUninstallCleanup() {
  ...
  const uninstallScript = path.join(
    os.tmpdir(),
    `desktop-app-uninstall-${Date.now()}.sh`,
  );
  ...
  const escapedLaunchAgent = launchAgentFile.replaceAll("'", "'\"'\"'");
...
NEW_LAUNCH_AGENT='${escapedLaunchAgent}'
...
launchctl bootout "gui/$(id -u)" "$NEW_LAUNCH_AGENT" >/dev/null 2>&1 || true
rm -f "$NEW_LAUNCH_AGENT" >/dev/null 2>&1 || true
...
async function openSystemSettingsPane(fragment) {
  ...
  try {
    ...
    const result = await execFileAsync('/usr/bin/open', [url]).catch(
      () => null,
    );
...
function showMemoryListWindow() {
  ...
  if (memoryListWindow.isMinimized()) {
    memoryListWindow.restore();
  }
  memoryListWindow.show();
  memoryListWindow.focus();
}

function createMemoryListWindow() {
  memoryListWindow = new BrowserWindow({
    width: 960,
    height: 720,
    minWidth: 720,
    minHeight: 560,
    title: '记忆列表 · Personal AI',
    icon: appIconPath,
    webPreferences: {
      preload: path.join(__dirname, 'preload.cjs'),
      contextIsolation: true,
  ...
  memoryListWindow.loadFile(path.join(__dirname, 'memory-list.html'));
  memoryListWindow.webContents.on('before-input-event', (event, input) => {
    if (!isToggleDevToolsShortcut(input)) return;
    event.preventDefault();
    toggleDevToolsForWindow(memoryListWindow);
  });
  memoryListWindow.webContents.setWindowOpenHandler(({ url }) => {
    void shell.openExternal(url);
    return { action: 'deny' };
  });
...
ipcMain.handle('bridge-app:open-memory-list-window', async () => {
  showMemoryListWindow();
  return { ok: true };
});
...
ipcMain.handle('quick-ask:set-layout', async (_event, payload) => {
  ...
  const maxHeight =
    screen.getDisplayMatching(askWindow.getBounds()).workArea.height -
    ASK_WINDOW_VERTICAL_MARGIN;
  const compactLikeMode = mode === 'compact' || mode === 'idle-compact';
  const voiceMode = mode === 'voice-listening' || mode === 'voice-ready';
  const nextHeight = compactLikeMode
    ? Math.max(
        ASK_WINDOW_COMPACT_HEIGHT,
        Math.min(
    ...
    : voiceMode
      ...
      : Math.max(
          ...
          Math.min(
            Number.isFinite(requestedHeight)
              ? requestedHeight
              : ASK_WINDOW_DEFAULT_EXPANDED_HEIGHT,
            maxHeight,
          ),
        );
...
app.on('activate', () => {
  // Tray/background app: do not auto-open Quick Ask or Settings. macOS fires
  // `activate` on many focus transitions; showing a window here felt like it
  // "popped up by itself". Use the tray, menu, or Option+A instead.
});
...
app.whenReady().then(async () => {
  ...
  const extensionIds = extensionIdsEnv
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean);
  if (extensionIds.length) {
    await installManifest(extensionIds).catch(async (error) => {
      await appendLog(
        appLogFile,
        `[error] native manifest install failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    });
  }
  applyMacUiMode();
  ...
  if (!backgroundMode) {
    createWindow();
    syncWindowPresence(true);
    showAskWindow({ focus: true, focusInput: true });
  } else {
    syncWindowPresence(false);
  }
  await appendLog(
    appLogFile,
    `Personal AI app started (background=${backgroundMode ? 'yes' : 'no'})`,
  );
});
