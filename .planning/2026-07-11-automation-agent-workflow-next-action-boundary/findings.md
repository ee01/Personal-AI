...
- Random sample from `docs/index.md` produced several recent near-neighbors; selected `Agent Workflow 运行诊断` as the first viable less-recent target with a bounded verifier path.
...
- `docs/features/message_analysis.md` is current and already documents local test scope, trace health, storageReview, evidence packets, saved baselines, and batch regression.
...
- Update `docs/features/message_analysis.md` and the feature index row to mention the `下一步` hover / reader boundary.
