...
- Added Options-language output for automatic briefs. `language_preference` now controls localized generated content and the composition version, while source names and evidence stay original; Lens brief labels, dates, actions, and receipts follow the generated brief language.
- Added a Chinese-to-English unchanged-evidence refresh test and focused browser coverage for both Chinese and English brief shells. Targeted backend tests pass 10/10, Keystone eval passes 7/7, webpack compiles, and the focused browser E2E passes.
- Deployed and backfilled production: all 9 `esone.qiu` automatic briefs now use `auto-reflection-grounded-v2-en-US`, matching the current Options language. Live group `160443817990` returns `NOVA-13680 Keystone Brief` with English summary and match reason.
