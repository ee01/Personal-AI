...
- Randomly selected `Agent Workflow 多 Agent 编排` from `docs/features/index.md`, excluding the freshest exact automation-memory feature families.
...
- Implemented pre-click batch baseline acceptance boundary copy in `src/options.tsx`, updated `tools/verify-agent-workflow-options-e2e.mjs`, and documented the behavior in `docs/features/agent_workflow.md`.
