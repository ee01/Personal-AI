...
Goal: improve the selected `主动询问 / 主动询问会话管理` feature from `docs/features/index.md` by checking current docs and code, grounding the UX decision in current product/research references, implementing one bounded low-decision improvement, and validating it through the strongest practical targeted checks.
