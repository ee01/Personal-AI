...
从 `docs/features/index.md` 随机选中 `四通道召回`。本轮不改 `RecallEngine` 检索、排序或写入路径，只优化搜索结果页里用户理解召回证据的路径。
...
4. 更新 `docs/features/memory_system.md`，记录该回执只是本地可见结果摘要，不重新召回、重排、写反馈或确认事实。
