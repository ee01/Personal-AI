...
随机抽中 `docs/features/index.md` 里的 `四通道召回`。本轮只做结果页呈现层改进：当搜索只返回 1 条可见结果时，也显示证据通道交叉/边界回执，避免用户把一个单通道命中误读成多通道交叉验证。
...
5. 简要更新 `docs/features/memory_system.md`，不改召回算法、排序、反馈写入或 Memory Service API。
