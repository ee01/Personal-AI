...
- `docs/features/compose_assist.md` already documents the current product boundary, host surfaces, trigger lifecycle, icon/display gates, append-vs-replace modes, stale-draft protection, preview-only-body rule, feedback/calibration behavior, Web AI compiler logic, Jira estimate fallback, and Persona Projection contract.
...
- Canonical `docs/features/compose_assist.md` already contains the detailed Prompt Compiler and Persona Projection contracts, but it lacks the requested near-top plain-language decision summary. Add that concise summary before cleanup.
...
- Added a near-top plain-language decision flow and explicit input/source priority to `docs/features/compose_assist.md`.
