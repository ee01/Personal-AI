...
随机命中 `docs/index.md` 里的 `定时消息列表筛选`。本轮改进聚焦一个窄问题：`过滤掉仅发我的` 依赖本机 Google 账号推导出的 `esone.qiu`，但旧表、人工编辑或导入行可能保存为 `Esone Qiu` 或邮箱形式，用户打开筛选后会看到“仅发我的”没有隐藏对应行，误以为筛选或数据刷新失效。
