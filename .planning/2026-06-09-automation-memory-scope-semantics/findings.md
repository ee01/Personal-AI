...
- Randomly selected feature from `docs/index.md`: `工作/个人/全部范围语义`.
...
- `docs/memory_system.md` matches the current core contract: `/recall` and `/ask` accept `work` / `personal` / `both` / `all`, active recall defaults to `work`, passive `/context-recall` defaults to `all`, and client `both` links normalize to `all`.
