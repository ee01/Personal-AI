...
随机抽中的功能是 `自我反思线程`（`docs/features/memory_system.md`）。这次只改一个可验证的 UX 问题：线程列表和详情页能看到状态、下次时间、动作队列和研究记录，但用户不能快速判断系统接下来会不会自动推进、正在等谁、以及失败时应该从哪里恢复。
...
5. 更新 `docs/features/memory_system.md` 的自我反思说明。
