...
/* harmony export */   processNewMessage: () => (/* binding */ processNewMessage)
/* harmony export */ });
/* harmony import */ var _llm__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./llm */ "./src/llm.ts");
/* harmony import */ var _vectorStore__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./vectorStore */ "./src/vectorStore.ts");
...
const availableTools = {
  entityExtraction: {
    ...
    execute: async params => {
      return await (0,_entityExtraction__WEBPACK_IMPORTED_MODULE_2__.extractEntitiesToStore)(params.content, params.metadata);
    }
  },
  relationshipAnalysis: {
    ...
    execute: async params => {
      ...
      `;
      const relationshipData = await (0,_llm__WEBPACK_IMPORTED_MODULE_0__.callLLMJsonAPI)({
  ...
  historySearch: {
    ...
    execute: async params => {
      ...
      return await (0,_vectorStore__WEBPACK_IMPORTED_MODULE_1__.naturalLanguageQuery)(searchQuery, filters, {
        limit: 5,
        sort: {
          field: 'timestamp',
          order: 'desc'
  ...
  relevanceJudgment: {
    ...
    execute: async params => {
      // 获取相关的人物及项目信息
      const knownPeople = await (0,_vectorStore__WEBPACK_IMPORTED_MODULE_1__.getAllKnownPeople)();
      const knownProjects = await (0,_vectorStore__WEBPACK_IMPORTED_MODULE_1__.getAllKnownProjects)();
      ...
      `;
      return await (0,_llm__WEBPACK_IMPORTED_MODULE_0__.callLLMJsonAPI)({
...
class AgentCoordinator {
  ...
  async getAgents() {
    try {
      const {
        customAgents
      } = await chrome.storage.local.get('customAgents');
  ...
  async processMessage(message) {
    ...
    // 按优先级排序
    const sortedAgents = agents.filter(agent => agent.enabled).sort((a, b) => b.priority - a.priority);
    ...
    for (const agent of sortedAgents) {
      ...
      try {
        ...
        if (toolResults.replyAdviser) {
          result.replyAdvice = toolResults.replyAdviser.needsReply ? toolResults.replyAdviser.replyText : '';
        }
      } catch (error) {
...
async function processNewMessage(message) {
  ...
  if (processResult.shouldStore) {
    ...
    await (0,_vectorStore__WEBPACK_IMPORTED_MODULE_1__.storeMessage)(messageId, message.message_content, {
...
/* harmony export */   getToolDescriptions: () => (/* binding */ getToolDescriptions),
/* harmony export */   intelligentAgent: () => (/* binding */ intelligentAgent),
/* harmony export */   processMessage: () => (/* binding */ processMessage),
/* harmony export */   registerTool: () => (/* binding */ registerTool)
/* harmony export */ });
/* harmony import */ var _llm__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./llm */ "./src/llm.ts");
/* harmony import */ var _vectorStore__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./vectorStore */ "./src/vectorStore.ts");
...
registerTool({
  ...
  execute: async params => {
    ...
    if (params.projects?.length) {
      ...
      filters.entities.projects = params.projects.map(name => ({
        name,
        required: true
      }));
    }
    ...
    const result = await (0,_vectorStore__WEBPACK_IMPORTED_MODULE_1__.naturalLanguageQuery)(query, filters, {
    ...
    return result.results.documents.map((doc, index) => ({
  ...
  execute: async params => {
    ...
    try {
      ...
      if (params.issueId) {
        ...
        // 使用JQL参数生成缓存键
        const jqlParams = [params.project ? `project=${params.project}` : '', params.status ? `status=${params.status}` : '', params.keywords ? `keywords=${params.keywords}` : ''].filter(Boolean).join('&');
      ...
      if (params.issueId) {
        ...
        result = {
          ...
          comments: responseData.fields.comment.comments.splice(-3).map(comment => comment.author.displayName + ': ' + comment.body),
        ...
        const response = await fetch(searchUrl, {
          ...
          body: JSON.stringify({
            jql: jql,
            maxResults: 10,
            fields: ['summary', 'status', 'assignee', 'description', 'priority', 'issuetype', 'created', 'updated', 'duedate', 'reporter']
          })
        });
        if (!response.ok) {
          const errorText = await response.text();
...
class IntelligentAgent {
    ...
    try {
      ...
      while (currentState.actionCount < MAX_ACTIONS) {
        ...
        if (thoughtResult.tools && thoughtResult.tools.length > 0) {
          ...
          // 并发执行所有选择的工具
          const toolPromises = thoughtResult.tools.map(async toolId => {
  ...
  async analyzeMessage(message) {
    ...
    // 构建上下文信息
    const contextInfo = [`发送者: ${message.sender || '未知发送者'}`, message.team_name ? `群组名称: ${message.team_name}` : '', message.datetime ? `发送时间: ${message.datetime}` : '', message.current_user ? `当前用户: ${message.current_user}` : ''].filter(Boolean).join('\n');
    ...
    if (message.concernedItems && Array.isArray(message.concernedItems)) {
      filterRulesInfo = `
关注规则:
${message.concernedItems.map((item, i) => `- 规则${i + 1}: ${item.text || item}`).join('\n')}
      `;
    }
...
- 如果需要了解历史上下文，考虑使用historySearch
    `;
    try {
      const analysis = await (0,_llm__WEBPACK_IMPORTED_MODULE_0__.callLLMJsonAPI)({
        prompt: analysisPrompt,
        type: 'analysis'
      });
      return analysis;
  ...
  async think(state) {
    // 构建思考提示
    const toolDescriptions = Object.entries(toolRegistry).map(_ref => {
      let [id, tool] = _ref;
    ...
    const memoryContent = Object.entries(state.memory).map(_ref2 => {
      let [key, value] = _ref2;
      return `- ${key} [已执行]: ${JSON.stringify(value).substring(0, 500)}${JSON.stringify(value).length > 300 ? '...' : ''}`;
    }).join('\n');
    ...
    if (state.analysis && state.analysis.matchedRules && state.analysis.matchedRules.length > 0) {
      matchedRulesInfo = `
# 匹配的关注规则
${state.analysis.matchedRules.map((rule, i) => `- 规则: ${rule}
   原因: ${state.analysis.matchReasons && state.analysis.matchReasons[i] ? state.analysis.matchReasons[i] : '未提供'}`).join('\n')}
      `;
...
- 规则: ${state.message.matched_rule}
    ...
    if (state.groupContext && state.groupContext.allMessages && state.groupContext.allMessages.length > 0) {
      ...
      const otherMessages = state.groupContext.allMessages.map((msg, idx) => ({
        ...msg,
        messageIndex: idx
      })).filter((_, idx) => idx !== currentIndex).filter(msg => msg.team_id == state.message.team_id).map((msg, i) => {
        const analysis = state.groupContext.allAnalyses.find(a => a.messageIndex === msg.messageIndex);
        return `消息 #${i + 1}:
发送者: ${msg.sender || '未知'}
...
请用JSON格式返回你的思考过程和决定的下一步行动:
{
  ...
  "replyAdvice": "可能的回复建议"
    ...
    // 处理结果时，保留决策相关字段
    try {
      const thinkResult = await (0,_llm__WEBPACK_IMPORTED_MODULE_0__.callLLMJsonAPI)({
        prompt: thinkPrompt,
        type: 'think'
      });
      ...
      if (thinkResult.tools && Array.isArray(thinkResult.tools)) {
        thinkResult.tools = thinkResult.tools.filter(tool => (availableTools.includes(tool) || Object.keys(toolRegistry).includes(tool)) && !executedTools.includes(tool));
      }
   ...
   */
  async analyzeBatchMessages(messages, groupContext) {
...
${envConfig.ANALYZE_BY_GROUP ? '' : `所在群组: ${msg.team_name || '未知群组'}`}`;
    ...
    // 构建群组上下文信息
    const groupInfo = [`群组名称: ${groupContext.groupName || '未知群组'}`, `群组ID: ${groupContext.groupId || '未知ID'}`, groupContext.username ? `当前用户: ${groupContext.username}` : ''].filter(Boolean).join('\n');
    ...
    if (messages[0]?.concernedItems && Array.isArray(messages[0].concernedItems)) {
      filterRulesInfo = `
关注规则:
${messages[0].concernedItems.map((item, i) => `- 规则${i + 1}: ${item.text || item}`).join('\n')}
      `;
    }
...
`;
    try {
      const analysisResults = await (0,_llm__WEBPACK_IMPORTED_MODULE_0__.callLLMJsonAPI)({
   ...
   * 批量处理多条消息
   */
  async processBatchMessages(messages, groupContext, onGroupCompleted) {
    ...
    try {
      ...
      // 记录批量处理开始
      const batchStartStep = {
        timestamp: Date.now(),
        thought: `开始批量处理 ${messages.length} 条消息，${batchAnalysisResults.filter(r => r.needsProcessing !== false).length} 条需要进一步处理`,
        action: '初始化批量处理'
      };
      ...
      for (let i = 0; i < normalizedMessages.length; i++) {
        ...
        // 为需要处理的消息创建消息处理结果
        const messageThoughtProcess = [];
        const messageUsedTools = [];
        const result = {
          ...
          groupName: message.team_name || '',
          originalMessage: message,
          matchedRule: analysis.matchedRules && analysis.matchedRules.length > 0 ? analysis.matchedRules.join('; ') : '',
        ...
        result.enrichedData = {
          ...
          entities: currentState.memory.entityExtractor?.entities || result.enrichedData.entities || message.entities || {},
          relationships: currentState.memory.entityExtractor?.relationships || result.enrichedData.relationships || message.relationships || [],
          actions: currentState.memory.entityExtractor?.actions || result.enrichedData.actions || message.actions || [],
      ...
      const batchEndStep = {
        timestamp: Date.now(),
        thought: `完成批量处理 ${messages.length} 条消息，其中 ${results.filter(r => r.shouldStore).length} 条被存储，${results.filter(r => r.shouldNotify).length} 条需要通知。共调用 LLM ${groupContext.aggregateLlmCallCount} 次，估计使用 ${groupContext.aggregateLlmCallTokens} tokens，使用工具：${Array.from(usedTools).join(', ')}`,
        action: '完成批量处理'
      };
 ...
 */
async function processMessage(input, onEveryGroupCompleted) {
  // 检测输入格式
  ...
  if (format === 'message_groups') {
    ...
    if (envConfig.ANALYZE_BY_GROUP) {
      ...
      for (const group of input.messageGroups) {
        const groupMessages = group.posts.map(post => ({
    ...
    // 处理单个消息组
    const messages = input.posts.map(post => ({
      message_content: post.content,
      sender: post.sender,
      datetime: post.datetime,
...
/* harmony export */   analyzeMessagesInBackground: () => (/* binding */ analyzeMessagesInBackground)
/* harmony export */ });
...
/* harmony import */ var _utils__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./utils */ "./src/utils.ts");
/* harmony import */ var _vectorStore__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./vectorStore */ "./src/vectorStore.ts");
/* harmony import */ var uuid__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! uuid */ "./node_modules/uuid/dist/esm-browser/v4.js");
/* harmony import */ var _entityExtraction__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./entityExtraction */ "./src/entityExtraction.ts");
/* harmony import */ var _agentSystem__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./agentSystem */ "./src/agentSystem.ts");
/* harmony import */ var _intelligentAgent__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./intelligentAgent */ "./src/intelligentAgent.ts");
...
async function analyzeMessages(data, username) {
  let isScheduledTask = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  try {
...
async function analyzeMessagesInBackground(data, username) {
  let isScheduledTask = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  // 获取环境配置
  const envConfig = await (0,_utils__WEBPACK_IMPORTED_MODULE_2__.getEnvConfig)();
  ...
  if (envConfig.ANALYSIS_TYPE === 'agentThinking') {
    ...
    try {
      ...
      const allResults = await (0,_intelligentAgent__WEBPACK_IMPORTED_MODULE_6__.processMessage)({
      ...
      for (const result of resultsArray) {
        ...
        if (result.shouldStore) {
          try {
            const originalMessage = result.originalMessage || {};
            const messageId = (0,uuid__WEBPACK_IMPORTED_MODULE_7__["default"])();
            ...
            // 存储消息
            await (0,_vectorStore__WEBPACK_IMPORTED_MODULE_3__.storeMessage)(messageId, originalMessage.message_content || '', {
    ...
    // 使用原有Agent系统处理消息
    const processResult = await (0,_agentSystem__WEBPACK_IMPORTED_MODULE_5__.processNewMessage)(data);
    ...
    if (envConfig.ENABLE_BOT) {
      ...
      (0,_bot__WEBPACK_IMPORTED_MODULE_0__.sendBotMessage)({
        matched_rule: processResult.matched_rule || '',
        // 使用可选链或提供默认值
...
async function processMessageFilterByConcernedItems(data, concernedItems, username, isScheduledTask) {
  const envConfig = await (0,_utils__WEBPACK_IMPORTED_MODULE_2__.getEnvConfig)();
  const system_prompt = `
...
async function reviewMessageByLLMAndSendToBot(body) {
  ...
  try {
    ...
    const dealResponse = await (0,_llm__WEBPACK_IMPORTED_MODULE_1__.callLLMJsonAPI)(body);
    ...
    if (dealResponse && dealResponse.data && dealResponse.data.length > 0) {
      for (const json of dealResponse.data) {
        ...
        if (envConfig.LLM_REVIEW_BEFORE_SEND) {
          // 先进行 LLM 审核
          const concernedItemsForPush = concernedItems.filter(item => item.pushToGlip);
...
${concernedItemsForPush.map((item, i) => `- 规则${i + 1}: ${item.text}`).join('\n')}
...
如果符合规则，请直接返回符合的规则原文，符合多条规则用换行隔开，不要包含其他内容。如果不符合任何规则，请返回"不通过"。
        ...
        // 原有逻辑：将匹配的消息存储到向量数据库
        const messageId = (0,uuid__WEBPACK_IMPORTED_MODULE_7__["default"])();
        const extractedEntities = await (0,_entityExtraction__WEBPACK_IMPORTED_MODULE_4__.extractEntitiesToStore)(json.message_content, json);
        await (0,_vectorStore__WEBPACK_IMPORTED_MODULE_3__.storeMessage)(messageId, json.message_content, {
...
var BasicAuthClientProvider = class {
  ...
  constructor(textCredentials) {
    const creds = textCredentials != null ? textCredentials : "MISSING_ENV_VAR".CHROMA_CLIENT_AUTH_CREDENTIALS;
    if (creds === void 0) {
      throw new Error(
...
var CloudClient = class extends ChromaClient {
  constructor({
    ...
    if (!apiKey) {
      apiKey = "MISSING_ENV_VAR".CHROMA_API_KEY;
    }
    if (!apiKey) {
...
var TransformersEmbeddingFunction = class _TransformersEmbeddingFunction {
  ...
  static async import() {
    try {
      const { pipeline } = await __webpack_require__.e(/*! import() */ "node_modules_xenova_transformers_src_models_js").then(__webpack_require__.bind(__webpack_require__, /*! @xenova/transformers */ "./node_modules/@xenova/transformers/src/transformers.js"));
      return { pipeline };
    } catch (e) {
      throw new Error(
...
/* harmony import */ var _vectorStore__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./vectorStore */ "./src/vectorStore.ts");
/* harmony import */ var _llm__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./llm */ "./src/llm.ts");
/* harmony import */ var _embeddings__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./embeddings */ "./src/embeddings.ts");
/* harmony import */ var _utils__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./utils */ "./src/utils.ts");
/* harmony import */ var _jira__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./jira */ "./src/jira.ts");
...
chrome.runtime.onInstalled.addListener(async () => {
  try {
    ...
    // 加载配置
    const config = await (0,_utils__WEBPACK_IMPORTED_MODULE_4__.getEnvConfig)();
    console.log('Global config loaded:', config);

    // 初始化配置
    const {
      scheduleActive
    } = await chrome.storage.local.get(['scheduleActive']);
    // 如果之前是激活状态，重新启动定时任务
    if (scheduleActive) {
      startScheduledCheck();
    }
    ...
    if (concernedItems) {
      // 过滤掉过期的项目
      const validItems = concernedItems.filter(item => {
    ...
    if (!concernedItems || concernedItems.length === 0) {
      chrome.storage.local.set({
        concernedItems: [{
          ...
          text: '可能是回复我的消息，比如在我发完消息之后的答复。排除发送者是我',
          pushToGlip: true
        }]
      });
    }
    ...
    // 安全地初始化Chroma
    try {
      if (await (0,_vectorStore__WEBPACK_IMPORTED_MODULE_1__.initChromaClient)()) console.log('Chroma client initialized');
    } catch (error) {
      console.error('Failed to initialize Chroma:', error);
    }

    // 预先创建离屏文档
    await (0,_embeddings__WEBPACK_IMPORTED_MODULE_3__.createOffscreenDocument)();
  } catch (error) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.type === 'MESSAGE_DEALING') {
    ...
    (0,_messageDealing__WEBPACK_IMPORTED_MODULE_0__.analyzeMessagesInBackground)(body.data, body.username, body.isScheduledTask).then(raw => {
      sendResponse({
        data: raw
      });
    });
  ...
  if (request.type === 'KNOWLEDGE_QUERY') {
    (0,_llm__WEBPACK_IMPORTED_MODULE_2__.knowledgeQuery)(request.question).then(result => {
  ...
  if (request.type === 'GET_ENV_CONFIG') {
    (0,_utils__WEBPACK_IMPORTED_MODULE_4__.getEnvConfig)().then(config => {
      sendResponse({
        success: true,
        config: config
      });
  ...
  // 监听来自离屏文档的消息
  (0,_embeddings__WEBPACK_IMPORTED_MODULE_3__.handleEmbeddingResult)(request);
  ...
  if (request.type === 'FETCH_JIRA_TICKETS') {
    ...
    (0,_jira__WEBPACK_IMPORTED_MODULE_5__.FETCH_JIRA_TICKETS)(jql, requestId, sender.tab?.id);
...
async function runScheduledTask() {
  ...
  try {
    ...
    await (0,_messageDealing__WEBPACK_IMPORTED_MODULE_0__.analyzeMessages)(response.data, userinfo.fullName, true);
  } catch (error) {
...
function sendMessageWithRetry(tabId, message) {
  ...
  let retryInterval = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 10000;
  return new Promise((resolve, reject) => {
    let attempts = 0;
