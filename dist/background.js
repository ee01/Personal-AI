...
async function analyzeMessages(data, config) {
  ...
  console.log(data, concernedItems, config);
  // 插入调试数据
  // data.unshift({
  ...
  if (true) {
    ...
    data.forEach(async (item, index) => await setTimeout(async () => {
      ...
      const prompt = `
            ...
            `;
      await sendMessageToLLM(prompt, item);
      chrome.storage.local.set({
        ollamaAnalysisProgress: {
          total: data.length,
...
const sendMessageToLLM = async (prompt, messageData) => {
